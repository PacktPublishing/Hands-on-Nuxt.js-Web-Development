const pkg = require('./package')


module.exports = {
  mode: 'universal',

  /*
  ** Headers of the page
  */
  head: {
    title: pkg.name,
    meta: [
      { charset: 'utf-8' },
      { name: 'viewport', content: 'width=device-width, initial-scale=1' },
      { hid: 'description', name: 'description', content: pkg.description }
    ],
    link: [
      { rel: 'icon', type: 'image/x-icon', href: '/favicon.ico' }
    ]
  },

  // https://nuxtjs.org/api/configuration-env#the-env-property
  env: {
    // This lets you create a baseUrl property that will be equal to the
    // BASE_URL environment variable if defined, otherwise, equal to
    // 'http://localhost:3000'
    baseUrl: process.env.BASE_URL || 'http://127.0.0.1:3000'
  },

  /*
  ** Customize the progress-bar color
  */
  loading: { color: '#fff' },

  /*
  ** Global CSS
  */
  css: [
  ],

  /*
  ** Plugins to load before mounting the App
  */
  plugins: [
  ],

  /*
  ** Nuxt.js modules
  */
  modules: [
    '@nuxtjs/axios',
    '@nuxtjs/auth'
  ],

  axios: {
    // baseURL: 'http://localhost:3030/',
    proxy: true
  },

  proxy: {
    // This will be your APIs base url
    '/api/': { target: 'http://localhost:3030/', pathRewrite: {'^/api/': ''} },
  },

  auth: {
    strategies: {
      // https://auth.nuxtjs.org/schemes/local.html#options
      local: {
        endpoints: {
          login: { url: '/api/public/login/local', method: 'post', propertyName: 'data' },
          user: { url: '/api/private/users', method: 'get', propertyName: 'data' },
          logout: false
        },
        tokenRequired: true,
        tokenType: 'bearer',

      },
      google: {
        client_id: '472476574621-01lgfg7fd3jl35sdne1r71n15r0mi244.apps.googleusercontent.com'
      },
      github: {
        client_id: 'c9564f8e50fec82c2619',
        client_secret: '36d77566a0c3f333bb19b6f1b176d3f6cc62d8cc'
      },
    },
    // https://auth.nuxtjs.org/api/options.html#redirect
    redirect: {
      // Redirect to [...] after login from the provider, e.g. Google.
      // Must match the settings in the Google app you created.
      callback: '/login',
      // Redirect to [...] after logout.
      logout: '/login',
      // Redirect to [...] after login.
      home: '/login'
    },

    // https://auth.nuxtjs.org/api/options.html#watchloggedin
    // When enabled (default) user will be redirected on login/logouts.
    // But it has some bugs on callback redirect above, so turn this off.
    // watchLoggedIn: false
  },

  /*
  ** Build configuration
  */
  build: {
    /*
    ** You can extend webpack config here
    */
    extend(config, ctx) {
    }
  }
}
