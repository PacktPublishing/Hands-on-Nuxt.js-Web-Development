...
import promise from './utils/await-catcher'
...
router.get('/users', async (ctx, next) => {
  ...
  try {
    ...
    // Find all docs.
    result = await collectionUsers.find({
    }, {
      //Exclude the fields.
...
router.get('/users/:id', async (ctx, next) => {
  ...
  // Or use a wrapper to catch the error.
  // Get the connection.
  var { error, data } = await promise(mongo.connect())
  if (error) {
    ctx.throw(500, error)
  }

  // Get the collection.
  const collectionUsers = data.collection('users')

  // console.log(typeof id) // string
  // console.log(typeof mongo.objectId(id)) // object

  // var object = mongo.objectId(id)
  // var timestamp = object.getTimestamp()
  // console.log(timestamp)

  var { error, data } = await promise(
